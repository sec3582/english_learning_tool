...
// ===== 同步狀態（國考工具模式） =====
// - dirty：本機有未同步變更的筆數（粗略計數即可）
// - lastSyncAt：上次成功「雲端同步」的時間戳
function readSyncMeta_(){
  return readJSON("syncMeta", { dirty: 0, lastSyncAt: 0 });
}
function writeSyncMeta_(m){
  writeJSON("syncMeta", m);
}
export function getSyncMeta(){
  return readSyncMeta_();
}
export function markDirty(n = 1){
  const m = readSyncMeta_();
  m.dirty = Math.max(0, Number(m.dirty || 0) + Number(n || 0));
  writeSyncMeta_(m);
}
export function clearDirtyAndSetLastSync(ts = Date.now()){
  const m = readSyncMeta_();
  m.dirty = 0;
  m.lastSyncAt = Number(ts || Date.now());
  writeSyncMeta_(m);
}
export function setLastSyncAt(ts = Date.now()){
  const m = readSyncMeta_();
  m.lastSyncAt = Number(ts || Date.now());
  writeSyncMeta_(m);
}
