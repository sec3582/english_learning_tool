...
## 1. Cinematic Grammar（全站鏡頭語法，14 條）

這 14 條是整個 Day Mode 的「電影拍攝規則」——任何元件若違反其中一條，即是跑出電影之外。
...
**帳冊橫線的技術特徵（for Phase 3：工程規格見 compiled-spec.md）：**
...
**方案選擇規則（for Phase 3：工程規格見 compiled-spec.md）：**
...
- **Ledger line 不算 border；input groove border 是另一套語意**：帳冊橫線（暖棕透明水平線）屬於 CG-13「Ledger Line」類別，是物件不是邊界；input/textarea 的四邊 groove border 是 CG-13 的書寫槽凹槽特例。兩者都不違反「禁止描邊深度」原則。
